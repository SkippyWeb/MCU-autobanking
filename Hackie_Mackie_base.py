...
midiInputVirtual = 'loopMIDI Port IN'
midiOutputVirtual = 'loopMIDI Port OUT'

for port in mido.get_input_names():
    if re.match(midiInput, port):
        midiInput = port
    if re.match(midiInputVirtual, port):
        midiInputVirtual = port

for port in mido.get_output_names():
    if re.match(midiOutput, port):
        midiOutput = port
    if re.match(midiOutputVirtual, port):
        midiOutputVirtual = port
